...
class LoshuGrid:
    """
    This class represents a Lo Shu Grid, which is a 3x3 grid used in numerology.
    """

    def __init__(self, dob: str, gender: str):
        """
        Initializes a new LoshuGrid with the given date of birth (dob) and gender.
        The grid is a dictionary where the keys are numbers from 1 to 9 and the values are initially empty strings.
        The grid is then populated with the dob and gender.
        """
    ...
    def populate_grid(self, dob: str, gender: str):
        """
        Populates the grid with the given date of birth (dob) and gender.
        It first splits the dob into day, month, and year, and calculates the psychic, destiny, and kua numbers.
        Then it splits the dob into individual digits, and if necessary, adds the psychic, destiny, and kua numbers to the digits.
        Finally, it adds each digit to the grid if the digit is not zero.
        """
...
class LoshuGridPlot:
    """
    This class is responsible for plotting a Lo Shu Grid using matplotlib.
    """
    ...
    def draw_loshu_grid(grid, celestial=False, labels=False, ax=None):
        ...
        If celestial is True, it adds celestial bodies to the grid.
        If labels is True, it adds row and column labels to the grid.
        The ax parameter can be used to draw on a specific Axes object.
        """
        if ax is None:
